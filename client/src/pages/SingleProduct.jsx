...
const SingleProduct = () => {
  ...
  // ✅ Updated button handlers to include quantity
  const handleAddToCart = () => {
    if (!selectedVariant) return;
    addToCart(product, selectedVariant, quantity); // ✅ pass quantity
  };
    ...
    <div className="bg-green-50 min-h-screen px-6 py-10 flex justify-center">
      <div className="w-full max-w-7xl grid grid-cols-1 lg:grid-cols-5 gap-10">
        ...
        <div className="lg:col-span-3 flex flex-col items-center">
          ...
          <div className="grid grid-cols-3 gap-4 mt-6 w-full max-w-md">
            {(product.images && product.images.length > 0
              ...
              <img key={idx} src={img} alt="" className="rounded-md shadow" />
            ))}
          </div>
        </div>
        ...
        <div className="lg:col-span-2 flex flex-col space-y-6">
          ...
          {/* Buttons */}
          <button
            className="w-full py-3 border border-yellow-600 text-yellow-700 font-medium rounded shadow hover:bg-yellow-50"
            onClick={handleAddToCart}
          >
            Add to Cart
          </button>
          <button
            className="w-full py-3 bg-yellow-600 text-white font-medium rounded shadow hover:bg-yellow-700"
